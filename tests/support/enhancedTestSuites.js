...
import {
  ...
  CommonTestData
} from './consolidatedTestSuites.js';
...
export const T = {
  ...
  suite: (type, ...args) => {
    switch(type) {
      case 'truth': return EnhancedTestSuites.dataModel(TestFactories.createTruthTestSuite(...args));
      case 'task': return EnhancedTestSuites.dataModel(TestFactories.createTaskTestSuite(...args));
      case 'lifecycle': return EnhancedTestSuites.lifecycle(args[0]);
      case 'nar': return EnhancedTestSuites.narIntegration(args[0]);
      case 'error': return EnhancedTestSuites.errorHandling(args[0]);
      default: throw new Error(`Unknown suite type: ${type}`);
    }
  }
};
