...
describe('OperationEvaluationEngine', () => {
  ...
  test('should handle error during functor execution', async () => {
    engine.addFunctor('errorFunctor', () => { throw new Error('Test error'); }, { arity: 0 });
    const opTerm = termFactory.create({ operator: '^', components: [
      'errorFunctor',
  ...
  test('should handle complex terms with variable substitution', async () => {
    ...
    const complexTerm = termFactory.create({ operator: '^', components: [
      'add',
  ...
  test('should handle nested compound term evaluation', async () => {
    ...
    const nestedTerm = termFactory.create({ operator: '^', components: [
      'add',
      { operator: ',', components: [
