...
export class TemporalDeductionRule extends NALRule {
    constructor() {
        super('temporal-deduction', {
            ...
            description: 'Performs temporal deduction: If <a =/> b> and <b =/> c> then <a =/> c>',
            priority: 0.5,
            category: 'temporal'
        });
    }
    ...
    _matches(task, context) {
        return task.term?.isCompound &&
               (task.term.operator === '=/' || task.term.operator === '/=' || task.term.operator === '<=>') && // Sequential, concurrent, or temporal similarity
